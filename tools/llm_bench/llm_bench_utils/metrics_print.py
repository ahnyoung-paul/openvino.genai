...
def print_metrics(
    ...
    is_chat_mode = chat_idx is not None
    chat_str = f"[C{chat_idx}]" if is_chat_mode else ""
    prefix = f"[{iter_str}]{chat_str}[P{prompt_idx}]"
    if batch_size > 1:
...
def print_memory_info(
    ...
    if iter_data["max_sys_mem_consumption"] != "" and iter_data["max_sys_mem_consumption"] > -1:
        output_str += f"max system memory cost: {iter_data['max_sys_mem_consumption']:.2f}MBytes, "
    ...
    if output_str != "":
        ...
        if iter_num == 0:
            iter_str = "warm-up"
        chat_str = f"[C{chat_idx}]" if chat_idx is not None else ""
        prompt_idx = f"[P{prompt_idx}]" if prompt_idx is not None else ""
        prefix = f"[{iter_str}]{chat_str}{prompt_idx}"
        output_str = " ".join([prefix, output_str])
        log.info(output_str)


def print_generated(iter_num, warm_up=False, generated=None, prompt_idx=-1, chat_idx=None):
    iter_str = str(iter_num)
    if warm_up:
        iter_str = "warm-up"
    prefix = f"[{iter_str}]"
    prefix += f"[C{chat_idx}]" if chat_idx is not None else ""
    prefix += f"[P{prompt_idx}]" if prompt_idx != -1 else ""
    if generated is not None:
        print_unicode(f"{prefix} Generated: {generated}", f"{prefix} Unable print generated")
...
def output_avg_statis_tokens(
    ...
    iter_alias = "C" if chat_mode else "P"
    for iter_idx in inputs_idx_list:
        ...
        for iter_data in iter_data_list:
            ...
            if iter_data['iteration'] == 0:
                continue
            if (chat_mode and iter_data["chat_idx"] == iter_idx) or (
                not chat_mode and iter_data["prompt_idx"] == iter_idx
            ):
                avg_1st_token_latency += (
                    iter_data["first_token_latency"] if iter_data["first_token_latency"] != "" else 0
                )
        ...
        if index_num > 0:
            ...
            prefix = (
                f"[ INFO ] [Average] {iter_alias}[{iter_idx}]L[{loop_idx}]"
                if loop_idx != -1
                else f"[ INFO ] [Average] {iter_alias}[{iter_idx}]"
            )
            if is_text_gen:
                output_info = ''
                if avg_input_size > 0:
                    ...
                    output_info = (
                        f" Input token sizes: {sum(input_sizes)}({', '.join(str(in_s) for in_s in input_sizes)}),"
                    )
                ...
                if avg_input_size > 0:
                    output_info += f" Input token size: {avg_input_size},"
                inputs_dict[iter_idx] = (
                    "\n{}{} 1st iteration latency: {}, 2nd iteration latency: {}, 2nd iteration throughput: {}".format(
                        prefix, output_info, avg_1st_token_latency, avg_2nd_tokens_latency, avg_2nd_token_tput
                    )
                )
            else:
                inputs_dict[iter_idx] = (
                    ...
                    "2nd steps of unet/transformer latency: {}, 2nd steps throughput: {}".format(
                        prefix, avg_1st_token_latency, avg_2nd_tokens_latency, avg_2nd_token_tput
                    )
                )
