...
int main(int argc, char **argv)
{
  try
  {
    ...
    else
    {
      ...
      for (int32_t input_idx : sentences_to_run) {
        auto input_text = sentences[input_idx];
        std::stringstream ss;
        // Build input prompt with prompt template
        std::cout << "******************************************* Text Sentence #" << input_idx << ", " << sentence_num << " Start *******************************************\n";
        ss << input_idx << ",";
        startTime = Time::now();
        ...
        for (int i = 0; i < args.num_iteration; i++)
        {
          ...
          std::cout << "******************************************* Text Sentence #" << input_idx << " Finished ****************************************\n\n";
